...
--fix
select HD.maHD,HD.ngayTaoHD,NV.tenNV,KH.tenKH,format(sum(CT.soLuongDat * SP.donGiaBan),'##,#\ VNĐ','es-ES')  
from SanPham as SP, ChiTietHoaDon as CT, HoaDon as HD, NhanVien as NV, KhachHang as KH
where SP.maSP = CT.maSP and CT.maHD = HD.maHD  and HD.maKH = KH.maKH and HD.maNV = NV.maNV
GROUP BY HD.maHD, HD.ngayTaoHD,NV.tenNV,KH.tenKH
having sum(CT.soLuongDat * SP.donGiaBan) > 2000000 and sum(CT.soLuongDat * SP.donGiaBan) > 10000000

select maNV, tenNV, email, chucVu, format(luong,'##,#\ VNĐ','es-ES') as N'Thành tiền' from NhanVien
where chucVu = 'Thu Ngân' and luong < 2000000

SELECT dbo.HoaDon.maHD, dbo.HoaDon.ngayTaoHD, dbo.KhachHang.tenKH, dbo.NhanVien.tenNV, format(SUM(dbo.ChiTietHoaDon.soLuongDat * dbo.SanPham.donGiaBan),'##,#\ VNĐ','es-ES') as N'Thành tiền'
FROM     dbo.ChiTietHoaDon INNER JOIN
                  dbo.HoaDon ON dbo.ChiTietHoaDon.maHD = dbo.HoaDon.maHD INNER JOIN
                  dbo.KhachHang ON dbo.HoaDon.maKH = dbo.KhachHang.maKH INNER JOIN
                  dbo.NhanVien ON dbo.HoaDon.maNV = dbo.NhanVien.maNV INNER JOIN
                  dbo.SanPham ON dbo.ChiTietHoaDon.maSP = dbo.SanPham.maSP
WHERE dbo.NhanVien.tenNV like '%trang%'
GROUP BY dbo.HoaDon.maHD, dbo.HoaDon.ngayTaoHD, dbo.KhachHang.tenKH, dbo.NhanVien.tenNV
Having SUM(dbo.ChiTietHoaDon.soLuongDat * dbo.SanPham.donGiaBan) > 10 and SUM(dbo.ChiTietHoaDon.soLuongDat * dbo.SanPham.donGiaBan) < 100000000
